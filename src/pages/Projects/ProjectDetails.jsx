...
export default function ProjectDetails() {
  ...
  const projectSkills = projectDetails.skills.map((skill, index) => {
    return (
      <p key={index} className={getSkillClass(index)}>
        {skill}
      </p>
    );
  });
  ...
  return (
    <>
      ...
      <main key={projectDetails.id} className="project-details">
        ...
        >
          Visit GitHub Repository <FaArrowRight className="icon" />
        </Link>
        {projectDetails.liveUrl != "" && (
          ...
          >
            Visit live application <FaArrowRight className="icon" />
          </Link>
        )}

        <div className="skills-container">{projectSkills}</div>
      </main>
    </>
  );
}
